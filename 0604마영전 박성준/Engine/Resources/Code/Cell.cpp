...
_bool Engine::CCell::Compare_Point(const _vec3* pPointFirst, 
...
void Engine::CCell::Render_Cell(void)
{
	...
	// m_pD3DXLine->Draw(); // 2d라인을 그리기 위한 함수
							...
							const _vec3* pPointC)
{
	...
	if (FAILED(pInstance->Ready_Cell(dwIndex, byOption, pPointA, pPointB, pPointC)))
...
Engine::CCell::COMPARE Engine::CCell::Compare(const _vec3* pEndPos, _ulong* pIndex, _vec2* NormalInLine)
{
	for (_ulong i = 0; i < LINE_END; ++i)
	{
		if (CLine::COMPARE_LEFT == m_pLine[i]->Compare(&_vec2(pEndPos->x, pEndPos->z)))
		{
			if (nullptr == m_pNeighbor[i])
			{
				NormalInLine->x = m_pLine[i]->Get_Normal().x;
				NormalInLine->y = m_pLine[i]->Get_Normal().y;
				return CCell::COMPARE_STOP;
			}
