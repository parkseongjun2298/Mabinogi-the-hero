...
_float Engine::CCalculator::Compute_HeightOnTerrain(const _vec3* pPos, const _vec3* pTerrainVtxPos, const _ulong& dwCntX, const _ulong& dwCntZ)
{
	_ulong	dwIndex = _ulong(pPos->z / 1.f) * dwCntX + _ulong(pPos->x / 1.f); // 나누기 1을 하는 이유는 지형의 인터벌이 1이기 때문이다.
...
_bool Engine::CCalculator::Collision_AABB(const _vec3* pDestMin, 
...
_bool CCalculator::Collision_OBB(const _vec3 * pDestMin, const _vec3 * pDestMax, const _matrix * pDestWorld, const _vec3 * pSourMin, const _vec3 * pSourMax, const _matrix * pSourWorld)
{
	...
	_float	fDistance[3]{};	// 0번 : 첫 번째 0bb에서 중점을 향하는 벡터를 임의의 축으로 투영한 길이의 합
							// 1번 : 두 번째 0bb에서 중점을 향하는 벡터를 임의의 축으로 투영한 길이의 합
							// 2번 : 중점을 이은 선분을 임의의 축으로 투영한 길이
	for (_uint i = 0; i < 2; ++i)
	{
		for (_uint j = 0; j < 3; ++j)
		{
