...
void Engine::CQuadTree::Culling_ForTerrain(CFrustum* pFrustum, const _vec3* pVtxPos, INDEX32* pIndex, _ulong* pTriCnt)
{
	m_fRadius = D3DXVec3Length(&(pVtxPos[m_dwCorner[CORNER_LT]] - pVtxPos[m_dwCenter]));

	_bool	bIn = pFrustum->Is_InFrustum(&pVtxPos[m_dwCenter], m_fRadius);

	if (true == bIn)
	{
		//if (nullptr == m_pChild[0]) // 최하위 쿼드트리 노드라면
		...
		if (true == LevelOfDetail(pFrustum->Get_GraphicDev(), pVtxPos))
		{
			...
			if (true == bVtxInFrustum[0] ||
				...
				true == bVtxInFrustum[2])
			{
				if (bNeighborDraw[NEIGHBOR_BOTTOM] &&
					bNeighborDraw[NEIGHBOR_LEFT])
				{
					if (nullptr == m_pChild[0])
					{
						pIndex[*pTriCnt]._0 = m_dwCorner[CORNER_LT];
