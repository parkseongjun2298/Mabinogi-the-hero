...
PS_OUT	PS_COMPLETEUI(PS_IN In)
{
	PS_OUT		Out = (PS_OUT)0;

	Out.vColor = tex2D(BaseSampler, In.vTexUV);	// 2차원 텍스쳐로부터 uv좌표에 해당하는 색을 얻어오는 함수, 반환타입은 vector타입

	if (In.vTexUV.x > (g_fUv-1.f  ))
		Out.vColor.a = 0.f;
...
technique Default_Device
{
	...
	pass COMPLETEUI
	{
		...
		vertexshader = compile vs_3_0 VS_MAIN();
		pixelshader = compile ps_3_0 PS_COMPLETEUI();
	}
	pass ALPHA
	{
		alphablendenable = true;
	...
	vertexshader = compile vs_3_0 VS_MAIN();
	pixelshader = compile ps_3_0 PS_ALPHALOWHP();
	}
};
