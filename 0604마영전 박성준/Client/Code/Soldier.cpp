...
void CSoldier::Soldier_Mode()
{
	...
	case SORDIER_ATT:
		...
		break;
	case SOLDIER_RUN:
		m_pMeshCom->Set_Animation(13);
		break;
	case SOLDIER_KNOCKBACK:
...
HRESULT CSoldier::Add_Component(void)
{
	...
	// Mesh
	pComponent = m_pMeshCom = dynamic_cast<Engine::CDynamicMesh*>(Engine::Clone(RESOURCE_STAGE, L"Mesh_Soldier"));
...
CSoldier* CSoldier::Create(LPDIRECT3DDEVICE9 pGraphicDev, _ulong NaviIndex)
{
...
void CSoldier::Move_Soldier(const _float & fTimeDelta)
{
	//m_SoldierState = SOLDIER_RUN;
	if (m_SoldierState == SORDIER_ATT)
	{
		Att_Soldier(fTimeDelta);
	}
	if (m_SoldierState == SOLDIER_KNOCKBACK)
	{
		Damage_Soldier(fTimeDelta);
	}
	if (m_SoldierState == SOLDIER_RUN && m_fPlayer_BossDis >= 0.1f)
	{
		m_fSpeed = 2.f;
...
void CSoldier::Att_Soldier(const _float & fTimeDelta)
{
	

	_vec3 vRight = m_pTransformCom->m_vInfo[Engine::INFO_LOOK];

	if (m_pMeshCom->m_pAniCtrl->m_fAccTime >= 0.1f && m_pMeshCom->m_pAniCtrl->m_fAccTime < 1.6f)
	{

		m_fSpeed = 20.f;
		float TempSpeed = m_fSpeed*fTimeDelta;
		//공격모션할때만 충돌 체크
		CCollisionMgr::GetInstance()->Collision_PlayerToSoldier(m_pPlayer, this);
		m_pTransformCom->m_vInfo[Engine::INFO_POS] = Engine::CManagement::GetInstance()->Get_Navi()->Move_OnNaviMesh(
			&m_pTransformCom->m_vInfo[Engine::INFO_POS],
			&(vRight * TempSpeed),
			&m_NaviIndex);
...
void CSoldier::Damage_Soldier(const _float & fTimeDelta)
{
	if (!m_HitCheck)
	{
