...
HRESULT CRuinBlade::Ready_GameObject()
{
...
_int CRuinBlade::Update_GameObject(const _float& fTimeDelta)
{
	...
	float Temp = m_fBladeSpeed*fTimeDelta;
	//룩으로 쭉 가게
	m_pTransformCom->m_vInfo[Engine::INFO_POS] += m_vLook*Temp;
	...
	if (!m_bBladeCheck)
	{
		m_pRuinBladeEffect = CRuinBladeEffect::Create(m_pGraphicDev, *m_pTransformCom->Get_Info(Engine::INFO_POS), m_fSaveAngleY);
		Engine::Add_GameObject(L"GameLogic", L"CRuinBladeEffect", m_pRuinBladeEffect);
	...
	if (m_fSaveTime >= 0.1f) //이게 첨에 생성될때 0 0 0 가운데 에서 생성이되서 무조건 충돌이 일어나서 막아주는중
	{
		...
		{
			CCollisionMgr::GetInstance()->CollisionRuinBladeToSoldier(this, m_pSoldier);
		}
		if (m_pDog != nullptr )
		{
			CCollisionMgr::GetInstance()->CollisionRuinBladeToDog(this, m_pDog);//충돌어디서 불러야하지
		}
		if (m_pKnight != nullptr )
		{
			CCollisionMgr::GetInstance()->Collision_RuinBladeToKnight(this, m_pKnight);//충돌어디서 불러야하지
		}
		if (m_pGuarder != nullptr )
		{
			CCollisionMgr::GetInstance()->Collision_RuinBladeToGuarder(this, m_pGuarder);//충돌어디서 불러야하지
		}

		if (m_pBoss != nullptr )
		{
			CCollisionMgr::GetInstance()->CollisionRuinBladeToBoss(this, m_pBoss);
		}
...
void CRuinBlade::Render_GameObject(void)
{
...
HRESULT CRuinBlade::Add_Component(void)
{
...
HRESULT CRuinBlade::SetUp_ConstantTextTable(LPD3DXEFFECT & pEffect)
{
	_matrix		matWorld, matView, matProj;
...
CRuinBlade* CRuinBlade::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 Look, _vec3 Pos,float AngleY)
{
