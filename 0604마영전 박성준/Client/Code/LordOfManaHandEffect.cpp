...
CLordOfManaHandEffect::CLordOfManaHandEffect(LPDIRECT3DDEVICE9 pGraphicDev)
	: Engine::CGameObject(pGraphicDev), m_fAngleX(0.f) , m_iFlag(0),
...
CLordOfManaHandEffect::~CLordOfManaHandEffect(void)
{

}

HRESULT CLordOfManaHandEffect::Ready_GameObject(const _uint& iFlag)
{
...
_int CLordOfManaHandEffect::Update_GameObject(const _float& fTimeDelta)
{
	if (m_bDead)
		return DEAD_OBJ;
	...
	if (nullptr == m_pParentBoneMatrix)
	{
		...
		const Engine::D3DXFRAME_DERIVED*	pFrame = pPlayerMeshCom->Get_FrameByName("ValveBiped_Anim_Attachment_LH"); //ValveBiped_Anim_Attachment_RH
	...
	m_fAngleX +=0.8 * fTimeDelta;
	//m_fAngleX *= 0.2;
	m_pTransformCom->Rotation(Engine::ROT_X, m_fAngleX);
	m_pTransformCom->Rotation(Engine::ROT_Y, m_fAngleX);
	m_pTransformCom->Rotation(Engine::ROT_Z, m_fAngleX);
	
	return 0;
}

void CLordOfManaHandEffect::Render_GameObject(void)
{
...
void CLordOfManaHandEffect::Free(void)
{
	Engine::CGameObject::Free();
}

HRESULT CLordOfManaHandEffect::Add_Component(void)
{
...
HRESULT CLordOfManaHandEffect::SetUp_ConstantTable(LPD3DXEFFECT & pEffect)
{
...
CLordOfManaHandEffect* CLordOfManaHandEffect::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _uint& iFlag)
{
	CLordOfManaHandEffect*	pInstance = new	CLordOfManaHandEffect(pGraphicDev);
